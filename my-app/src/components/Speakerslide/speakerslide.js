...
function Speakers() {
  ...
  return (
    <div className="w-3/4 m-auto mt-[65px]">
      <div className="flex  justify-center align-center text-2xl font-bold text-red-700">
        Executive Speakers
      </div>
      <div className="mt-5">
        <Slider {...settings}>
          {speakers.map((s) => (
            <div
              key={s.name}
              className="bg-blue-800 h-10px pt-10 px-14 rounded-xl text-white"
            >
              <div className=" rounded-xl bg-blue-300 flex justify-center align-center py-5 px-5 mx-16">
                <img
                  src={s.img}
                  alt="speakerimg"
                  className="h-[450px] w-[640px] rounded-xl"
                ></img>
              </div>
